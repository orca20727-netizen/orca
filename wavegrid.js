// wavegrid.js — Wave Grid background (vanilla JS port of the Originkit
// component; no React/Framer dependency). Mechanism credit: franky-adl/3d-wave-grid.
...
class WaveGridScene {
  ...
  buildGrid() {
    ...
    this.offsets = new THREE.InstancedBufferAttribute(new Float32Array(count * 2), 2);
...
/**
 * Mount the Wave Grid into `container` (any block element with a defined
 * size). Returns { updateConfig(next), dispose() }, or null if WebGL isn't
 * available (fails silently so the page still renders fine).
 */
export function createWaveGrid(container, config = {}) {
  ...
  try {
    ...
    console.warn("WaveGrid: WebGL unavailable, skipping background effect.", err);
    return null;
  }
