...
class RoutePlanner:
    ...
    def plan_route(
        ...
        simplified = self._smooth_path(waypoints_latlon)

        # _smooth_path already only keeps edges it verified are clear, but
        # re-check the whole thing as a final safety net (e.g. against any
        # future change to the smoothing logic) and fall back to the dense
        # A* staircase path if anything slipped through.
    ...
    def _smooth_path(
        ...
        """
        Greedy line-of-sight ("string pulling") path smoothing.

        The raw A* path is a staircase: 8-connected movement can only step
        N/S/E/W/diagonal, so a route whose true heading is e.g. 30 degrees
        gets approximated as an alternating N/NE/N/NE zig-zag of grid hops.
        The previous approach only merged waypoints that were already
        near-collinear, which does nothing for a staircase -- consecutive
        hops in a zig-zag are *not* collinear with each other even though
        the overall corridor is obstacle-free and could be walked in one
        straight line.

        This instead repeatedly looks, from the current waypoint, for the
        *farthest* later waypoint it has a clear (unobstructed) direct line
        to -- using the same land/MPA edge check A* used to build the path
        -- and jumps straight there, skipping every intermediate grid hop.
        Applied along the whole path this turns the zig-zag staircase into
        a short sequence of real, mostly-straight route legs that still
        curve only where an actual obstacle (coastline, MPA) requires it.
        """
        if len(points) <= 2:
            return points
        ...
        while current_idx < n - 1:
            next_idx = n - 1
            # Walk backwards from the far end until we find a waypoint
            # that's directly, obstacle-free reachable from current_idx.
            while next_idx > current_idx + 1 and self._edge_blocked(
                points[current_idx][0], points[current_idx][1],
                points[next_idx][0], points[next_idx][1],
            ):
                next_idx -= 1
